...
def getToken():
    response = ''.join(open('temp.txt','r',encoding='utf-8').readlines())
    if '&token=' in response:
        print('curl get token ...')
        start = response.find('&token=') + 7
        end = start + 64
...
with open('iltv_template.m3u','r',encoding='utf-8') as f:
    ...
    with open('../m3u/iltv_bak.m3u','w',encoding='utf-8') as g:
        g.write(txt)

with open('iltv_template.txt','r',encoding='utf-8') as f:
    txt = f.read().replace('&token=key','&token='+str(token))
    with open('../m3u/iltv_bak.txt','w',encoding='utf-8') as g:
        g.write(txt)
